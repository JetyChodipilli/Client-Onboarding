...
export function InvitationPanel({ onboardingId, clientId, onboardingStatus }: {
  ...
  useEffect(() => {
    ...
    Promise.all([
        ...
        canInvite ? operationsApi.invitations(onboardingId) : Promise.resolve([]),
  ...
  return <section className="mt-8" aria-labelledby="client-access-heading">
    ...
    <div className="mt-4 space-y-3">
      {invitations.length === 0 ? <Alert>No invitation has been sent. Add a client contact first if this list is empty.</Alert> : invitations.map((invitation) => <Card key={invitation.id} className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
