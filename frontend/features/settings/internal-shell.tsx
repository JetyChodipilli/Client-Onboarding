...
export function InternalShell({ children }: { children: React.ReactNode }) {
  ...
  const signOut = async () => { await authApi.logout().catch(() => undefined); router.push("/login"); };
      ...
      <div className="min-h-dvh bg-background lg:grid lg:grid-cols-[17rem_minmax(0,1fr)]">
        <aside className="border-b bg-card/80 lg:sticky lg:top-0 lg:h-dvh lg:border-b-0 lg:border-r">
          <div className="flex min-h-20 items-center justify-between gap-3 px-5 lg:border-b">
            ...
            <div className="flex items-center gap-1 lg:hidden"><ThemeToggle /><Button size="icon" variant="ghost" aria-label="Sign out" onClick={signOut}><LogOut aria-hidden="true" /></Button></div>
          </div>
          ...
          <div className="hidden border-t p-4 lg:absolute lg:inset-x-0 lg:bottom-0 lg:block">
            ...
            <div className="mt-3 flex items-center justify-between"><ThemeToggle /><Button size="sm" variant="ghost" onClick={signOut}><LogOut aria-hidden="true" />Sign out</Button></div>
          </div>
        </aside>
